...
public:
				...
				if ( *socket == *m_server ){
					...
					if ( (bytes = socket->recv(buffer)) <= 0 ){
						...
						if ( select_value->has_terminator() ){
							...
							process_request(select_value->socket(), cmd);
						}
					}
				}
			}
	...
	void process_request(Socket *referrer, std::string const &request) {
		(void)referrer;
		std::vector<std::string> arguments = ft::split(request, " ");
